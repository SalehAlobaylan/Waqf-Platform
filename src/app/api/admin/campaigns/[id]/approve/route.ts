...
export async function POST(request: NextRequest, { params }: RouteParams) {
    ...
    return withApiHandler(request, "api.admin.campaigns.approve", async () => {
        const admin = await requireAdminOrThrow();
        ctx.userId = admin.id;

        const rate = checkRateLimit(request, "admin-campaign-approve", { limit: 30, windowMs: 60_000 }, admin.id);
        if (!rate.allowed) {
        ...
        try {
            ...
            log.warn("api.admin.campaigns.approve", "notify failed", undefined, err);
        }
