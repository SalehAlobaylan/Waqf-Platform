...
export async function GET(request: NextRequest) {
  return withApiHandler(request, "api.search", async () => {
    ...
    if (terms.length > 0) {
      ...
      try {
        ...
        // A tsquery parse failure must degrade to substring search instead of
        // taking down the endpoint.
        log.warn("api.search", "full-text query failed, falling back to ILIKE", { query, tsQuery }, error);
      }
    }

    // If full-text found nothing (or failed), fall back to substring matching
    // so partial terms still work.
